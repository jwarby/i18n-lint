  ...
  var parser = new htmlparser.Parser({
    ...
    ontext: function(text) {
      ...
      if (
        ...
        options.ignoreTags.indexOf(_.last(state.tagStack).name) === -1
      ) {
        if (_.last(state.tagStack).error) {
          if (state.lineNumber !== _.last(state.tagStack).error.error.line) {
            evidence = new RegExp('(' + utils.escapeRegExp(text) + ')');

            errors.push(extend(true, {}, _.last(state.tagStack).error, {
              error: {
                code: 'W001',
                reason: 'Hardcoded <' + _.last(state.tagStack).name + '> tag (continued)',
            ...
            evidence = _.last(state.tagStack).error.error.evidence;

            _.last(state.tagStack).error.error.evidence = new RegExp(
          ...
          errors.push(extend(true, {}, errorTemplate, {
            error: {
              code: 'W001',
              reason: 'Hardcoded <' + _.last(state.tagStack).name + '> tag',
              line: state.lineNumber,
              evidence: evidence,
              scope: originalLines[state.lineNumber - 1],
          ...
          _.last(state.tagStack).error = errors[errors.length - 1];
        }
      }
    },
    /**
