...
var error = function(code, reason, evidence, character, scope, line) {
  HSLINT.errors.push({
    ...
    line: line || HSLINT.lineNumber,

    // Column number within `line`
    character: character,

    // Contents of `line` from source file
    scope: scope || originalLines[HSLINT.lineNumber - 1].replace(/\r$/, '')
...
hslint.scan = function(lines, options) {
  ...
  var parser = new htmlparser.Parser({
    ...
    ontext: function(text) {
      ...
      var currentTag = _.last(HSLINT.tagStack);
      ...
      if (!currentTag) {
        ...
        HSLINT.tagStack.push(currentTag);
      }
      ...
      if (
        ...
        if (currentTag.error) {
           ...
           */
          if (HSLINT.lineNumber !== currentTag.error.line) {
            evidence = createEvidenceRegExp(text);

            // Report error
            error(
              'W001',
              currentTag.error.reason + ' (continued)',
              evidence,
          ...
          currentTag.error = error(
            'W001',
            'Hardcoded ' + (currentTag.name ? '<' + currentTag.name + '> tag' : 'text node'),
    ...
    onattribute: function(name, value) {
      ...
      if (
        ...
        scope = originalLines.slice(HSLINT.lineNumber - offset, HSLINT.lineNumber).join('\n');
